-- Authenticate before looking up a sheet and make deny-only RLS intent explicit.
create policy result_smart_sheets_rpc_only on public.result_smart_sheets
  for all to anon, authenticated using (false) with check (false);
create policy result_smart_recordings_rpc_only on public.result_smart_recordings
  for all to anon, authenticated using (false) with check (false);
...
as $function$
declare v_sheet public.result_smart_sheets%rowtype; v_auth jsonb; v_session jsonb;
begin
